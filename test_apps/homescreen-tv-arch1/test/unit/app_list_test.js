...
requireApp('homescreen-tv-arch1/js/app_list.js');
  ...
  suite('mock app loaded', function() {
    ...
    function createMockAppEntries(count) {
      for (var i = 0; i < count; i++) {
        dummyApps.push({
          'origin': 'https://test-' + i + '.gaiamobile.org',
          'entry_point': '',
          'name': 'test-app-' + i
        });
      }
    ...
    test('previous page', function() {
      assert.isFalse(appList.previousPage());
      assert.isTrue(appList.nextPage());
      assert.isTrue(appList.previousPage());
    });

    test('next page', function() {
      assert.isTrue(appList.nextPage());
      assert.isFalse(appList.nextPage());
    });

    test('press enter', function() {
      var spied = this.sinon.spy(MockApplications, 'launch');
      appList.handleKeyDown('Enter');
      assert.isTrue(spied.called);
      spied.restore();
    });

    test('press up/donw/left/right', function() {
      var spied = this.sinon.spy(MockSpatialNavigator.singleton, 'move');
      appList.handleKeyDown('Up');
      assert.equal(spied.callCount, 1);
      appList.handleKeyDown('Down');
      assert.equal(spied.callCount, 2);
      appList.handleKeyDown('Left');
      assert.equal(spied.callCount, 3);
      appList.handleKeyDown('Right');
      assert.equal(spied.callCount, 4);
      spied.restore();
    });

    test('add app', function() {
      var newApp = {
        'origin': 'https://new-app.gaiamobile.org',
        'entry_point': '',
        'name': 'new-app'
      };

      var spied = this.sinon.spy(MockSpatialNavigator.singleton, 'add');
    ...
    test('remove app', function() {
      var entry = {
        'origin': 'https://test-0.gaiamobile.org',
        'entry_point': '',
        'name': 'test-app-0'
      };

      var spied = this.sinon.spy(MockSpatialNavigator.singleton, 'remove');
